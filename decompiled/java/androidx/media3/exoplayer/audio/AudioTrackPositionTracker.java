...
final class AudioTrackPositionTracker {
    ...
    private long getPlaybackHeadPosition() {
        if (this.stopTimestampUs != C0565C.TIME_UNSET) {
    ...
    private void updateRawPlaybackHeadPosition(long j10) {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.rawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C0565C.TIME_UNSET) {
                    this.forceResetWorkaroundTimeMs = j10;
                    return;
                }
                return;
            }
            this.forceResetWorkaroundTimeMs = C0565C.TIME_UNSET;
        }
    ...
    public long getCurrentPositionUs() {
        ...
        if (zHasAdvancingTimestamp) {
            ...
            if (this.playheadOffsetCount == 0) {
                mediaDurationForPlayoutDuration = this.stopTimestampUs != C0565C.TIME_UNSET ? Util.sampleCountToDurationUs(getSimulatedPlaybackHeadPositionAfterStop(), this.outputSampleRate) : getPlaybackHeadPositionUs();
            } else {
            ...
            jMax = Math.max(0L, mediaDurationForPlayoutDuration - this.latencyUs);
            if (this.stopTimestampUs != C0565C.TIME_UNSET) {
        ...
        if (j10 < 1000000) {
            long mediaDurationForPlayoutDuration2 = Util.getMediaDurationForPlayoutDuration(j10, this.audioTrackPlaybackSpeed) + this.previousModePositionUs;
            long j11 = (j10 * 1000) / 1000000;
            jMax = (((1000 - j11) * mediaDurationForPlayoutDuration2) + (jMax * j11)) / 1000;
        }
    ...
    public boolean isStalled(long j10) {
        return this.forceResetWorkaroundTimeMs != C0565C.TIME_UNSET && j10 > 0 && this.clock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public void setAudioTrack(AudioTrack audioTrack, boolean z7, int i6, int i10, int i11) {
        ...
        this.hasData = false;
        this.stopTimestampUs = C0565C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C0565C.TIME_UNSET;
